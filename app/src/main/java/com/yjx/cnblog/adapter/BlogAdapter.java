...
public class BlogAdapter extends RecyclerView.Adapter<BlogAdapter.ViewHolder> {
    ...
    public BlogAdapter(Context context, List<InfoBean> mLists) {
        this.mLists = mLists;
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.tv_name.setText(mLists.get(position).getAuthor().getName());
        if (!TextUtils.isEmpty(mLists.get(position).getAuthor().getAvatar()))
            Picasso.with(context).load(mLists.get(position).getAuthor().getAvatar()).error(R.drawable.ic_launcher).into(holder.iv_header);
        else
